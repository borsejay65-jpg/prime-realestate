...
export default function TestimonialSlider() {
  ...
  useEffect(() => {
    const timer = setInterval(() => setActive(p => (p + 1) % demoTestimonials.length), 5000)
    return () => clearInterval(timer)
  }, [])

  const t = demoTestimonials[active]
    ...
    <section className="section bg-gray-50 dark:bg-gray-900/50">
      <div className="container-custom">
        ...
        <div className="max-w-3xl mx-auto relative">
          <button onClick={() => setActive(p => (p - 1 + demoTestimonials.length) % demoTestimonials.length)}
          ...
          <button onClick={() => setActive(p => (p + 1) % demoTestimonials.length)}
